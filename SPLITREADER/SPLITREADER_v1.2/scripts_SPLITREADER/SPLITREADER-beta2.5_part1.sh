...
if [ "$#" -ne 10 ] && [ "$#" -ne 9 ] && [ "$#" -ne 8 ]; then
  echo ""
  echo "Name : $0"
  echo ""
  echo "Usage: "
  echo "	$0 <bam file name> <bam file directory> <bam file extension> <cohort name> <working directory> <TE library> <script directory> <tmp directory> <LS: library size> <number of threads>"
  echo ""
  echo "Description:"
  echo ""
...
CORES=${10:-$CORES_DEFAULT} # Number of threads : By default is 2 threads
...
if [ -z "$pe" ]
  then
 	_JAVA_OPTIONS="-Xmx10g -XX:+UseSerialGC -Djava.io.tmpdir=$TmpDir/javatemp" picard SamToFastq --INPUT $TmpDir/${in}${ext}.bam --FASTQ $readsDir/$in.fastq --TMP_DIR $TmpDir/javatemp  2>> $TmpDir/log.txt
    
  else
	_JAVA_OPTIONS="-Xmx10g -XX:+UseSerialGC -Djava.io.tmpdir=$TmpDir/javatemp" picard SamToFastq --INPUT $TmpDir/${in}${ext}.bam --FASTQ $readsDir/$in.1 --SECOND_END_FASTQ $readsDir/$in.2 --TMP_DIR $TmpDir/javatemp  2>> $TmpDir/log.txt
  

echo "["$(date +"%y-%m-%d %T")"] Extracting discordant reads from ${in}${ext}: Insert size=$LS" | tee -a $TmpDir/log.txt

	samtools view -hF 4 $InputDir/${in}${ext}.bam | awk -v l=$LS '(($4-$8)>(l*10) || ($8-$4)>(l*10) || $7!="=") || $1~/@HD/ || $1~/@SQ/ || $1~/@PG/' | picard SamToFastq -INPUT /dev/stdin -FASTQ $readsDir/$in.1b -SECOND_END_FASTQ $readsDir/$in.2b -TMP_DIR $TmpDir/javatemp  2>> $TmpDir/log.txt
